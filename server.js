...
app.get('/call-token', async (req, res) => {
  ...
  if (!isMissionUsable(mission)) return res.status(410).json({ error: 'Mission terminée, appel impossible' });
  if (![mission.client_id, mission.depanneur_id].includes(userId)) {
...
app.post('/voice', async (req, res) => {
  ...
  if (!isMissionUsable(mission)) {
    twiml.say({ language: 'fr-FR' }, "Cette mission est terminée, l'appel n'est plus disponible.");
    return res.type('text/xml').send(twiml.toString());
  }
...
app.listen(PORT, () => console.log(`Remorqo VoIP backend (Postgres) actif sur le port ${PORT}`));
